...
import { explorerUrl, getChainEntry, requireCore, requireProtocol, } from "../chains.js";
import { publicClient } from "../clients.js";
import { requireWrite, simulateOrSend, spend, serializeBigints, getMode, } from "../safety.js";
import { getSigner } from "../signers/index.js";
import { PAYROLL_FACTORY_ABI, ERC20_ABI, PAYROLL_ADVANCE_ABI, ADAPTIVE_CADENCE_ABI, YIELD_ESCROW_ABI, COMPLIANCE_REGISTRY_ABI, PAYROLL_ATTESTOR_ABI, } from "../abis.js";
...
async function ctx(chainId) {
    ...
    return {
        ...
        pc,
        chain: entry.chain,
        explorerTxUrl: (hash) => explorerUrl(chainId, hash),
    };
}
export const writeTools = [
    {
        name: "hashpay_whoami",
        description: "Return the active signer account + mode + session spend snapshot. Helpful to confirm which wallet the MCP will sign as before running writes.",
    ...
    },
    {
        ...
        description: "Deploy a new payroll via PayrollFactory. Recipients array and amounts array must be equal length. Frequency is in seconds (e.g. 604800 for weekly). Returns the new payrollId on success.",
        inputSchema: z.object({
                ...
                for (const log of receipt.logs) {
                    try {
                        ...
                        if (decoded.eventName === "PayrollCreated") {
                            payrollId = decoded.args.payrollId.toString();
                            break;
                        }
                    }
                    catch {
    ...
    },
    {
        ...
        description: "Transfer tokens from the signer into payroll escrow. Approves the factory first if allowance is insufficient. Amount is in token base units (e.g. 1000000 = 1 USDC at 6 decimals).",
        inputSchema: z.object({
            chainId: ChainIdSchema,
        ...
        handler: async (args) => {
            const { chainId, payrollId, amount } = args;
    ...
    },
    {
        ...
        description: "Trigger settlement on a payroll. Runs compliance hooks → advance repayment → cadence routing → transfers. Anyone can call this (keeper-friendly). Returns tx hash.",
        inputSchema: z.object({ chainId: ChainIdSchema, payrollId: PayrollIdSchema }),
        handler: async (args) => {
            const { chainId, payrollId } = args;
            const c = await ctx(chainId);
    ...
    },
    {
        ...
        description: "Remove a recipient from a payroll by their index in the recipients array. Read the payroll first to find the index.",
        inputSchema: z.object({
            chainId: ChainIdSchema,
            payrollId: PayrollIdSchema,
    ...
    },
    {
        ...
        description: "Set a recipient's cadence mode on AdaptiveCadence. Modes: BATCH (lump-sum at cycle), STREAM (per-second), PULL (recipient claims), HYBRID (split BATCH/STREAM by hybridStreamBps).",
        inputSchema: z.object({
            chainId: ChainIdSchema,
            payrollId: PayrollIdSchema,
            recipient: AddressSchema,
    ...
    },
    {
        ...
        description: "Recipient claims their accrued-but-unclaimed streaming balance on AdaptiveCadence. Must be called by the recipient (the active signer).",
        inputSchema: z.object({ chainId: ChainIdSchema, payrollId: PayrollIdSchema }),
        handler: async (args) => {
            const { chainId, payrollId } = args;
            const c = await ctx(chainId);
    ...
    },
    {
        ...
        description: "Attach the YieldEscrow vault to a payroll so idle funds earn yield (ERC-4626 wrapper; MockYieldVault pays 4.5% APY on testnets). Only callable by payroll owner.",
        inputSchema: z.object({
            chainId: ChainIdSchema,
            payrollId: PayrollIdSchema,
    ...
    },
    {
        ...
        description: "Sweep accrued yield from the vault back into the payroll's spendable balance. Callable by payroll owner.",
        inputSchema: z.object({ chainId: ChainIdSchema, payrollId: PayrollIdSchema }),
        handler: async (args) => {
            const { chainId, payrollId } = args;
            const c = await ctx(chainId);
    ...
    },
    {
        ...
        description: "Borrow against expected future income. LTV and APR are priced to the recipient's reputation tier — check with hashpay_get_advance_quote first. Repaid automatically at the next executeCycle.",
        inputSchema: z.object({
            chainId: ChainIdSchema,
            payrollId: PayrollIdSchema,
            amount: AmountSchema,
    ...
    },
    {
        ...
        description: "Lend to the PayrollAdvance pool. You earn interest as recipients borrow against future wages. Returns minted pool shares. Approves token first if needed.",
        inputSchema: z.object({
            chainId: ChainIdSchema,
            token: AddressSchema,
            amount: AmountSchema,
        ...
        handler: async (args) => {
            ...
            if (fr.txHash)
                spend.record(a.amount);
            return serializeBigints({ chainId: a.chainId, steps });
        },
    },
    {
        name: "hashpay_attach_compliance_hook",
        description: "Attach a compliance hook (KYC, Jurisdiction, Sanctions, RateLimit, Timelock) to a payroll. Use hashpay_list_chains to discover hook addresses.",
        inputSchema: z.object({
            chainId: ChainIdSchema,
            payrollId: PayrollIdSchema,
            hook: AddressSchema,
    ...
    },
    {
        ...
        description: "Create EAS attestations for a settled cycle, recording income on-chain and updating each recipient's ReputationRegistry score. Call after hashpay_execute_cycle; requires the attestor service role.",
        inputSchema: z.object({
            chainId: ChainIdSchema,
            payrollId: PayrollIdSchema,
    ...
    },
    {
        ...
        description: "DEV-ONLY: mint MockUSDT on testnets. Decimals=6. Only works on chains where MOCK_USDT is deployed and the mint() function is permissionless (testnets).",
        inputSchema: z.object({
            chainId: ChainIdSchema,
            to: AddressSchema,
            amount: AmountSchema,

...
import { z } from "zod";
import { CHAIN_REGISTRY, addressUrl, allowedChainIds, requireCore, requireProtocol, } from "../chains.js";
import { publicClient } from "../clients.js";
import { serializeBigints } from "../safety.js";
import { PAYROLL_FACTORY_ABI, REPUTATION_REGISTRY_ABI, PAYROLL_ADVANCE_ABI, YIELD_ESCROW_ABI, ADAPTIVE_CADENCE_ABI, COMPLIANCE_REGISTRY_ABI, SALARY_INDEX_ABI, ERC20_ABI, } from "../abis.js";
import { AddressSchema, ChainIdSchema, PayrollIdSchema } from "../schemas.js";
export const readTools = [
    {
        name: "hashpay_list_chains",
        description: "List all chains supported by HashPay. For each chain returns status (deployed/undeployed), name, chainId, and deployed contract addresses.",
    ...
    },
    {
        ...
        description: "Read full state of a payroll: owner, token, name, recipients, amounts, frequency, cycle count, totals, active flag, and current escrow balance.",
        inputSchema: z.object({
        ...
        handler: async (args) => {
            ...
            return serializeBigints({
                ...
                amounts: details[4],
                frequencySeconds: details[5],
                startTime: details[6],
                lastExecuted: details[7],
                cycleCount: details[8],
                totalDeposited: details[9],
    ...
    },
    {
        ...
        description: "Returns all payroll IDs that include the given recipient address. Useful for agents checking who's paying them.",
        inputSchema: z.object({
            chainId: ChainIdSchema,
            recipient: AddressSchema,
        }),
        handler: async (args) => {
            const { chainId, recipient } = args;
            const factory = requireCore(chainId, "PAYROLL_FACTORY");
            const pc = publicClient(chainId);
    ...
    },
    {
        ...
        description: "Read reputation metrics for any address from ReputationRegistry. Returns cumulative income, distinct employer count, on-time payment rate (bps), and highest milestone reached. Use this to judge agent or contributor reliability.",
        inputSchema: z.object({
            chainId: ChainIdSchema,
    ...
    },
    {
        ...
        description: "Oracle-style predicate: returns true if the address received at least `minAmount` within the last `windowSeconds`. Useful for gating (lending decisions, visa attestations, premium feature access).",
        inputSchema: z.object({
            chainId: ChainIdSchema,
            address: AddressSchema,
        ...
        handler: async (args) => {
            const { chainId, address, minAmount, windowSeconds } = args;
            const reg = requireProtocol(chainId, "REPUTATION_REGISTRY");
            const pc = publicClient(chainId);
    ...
    },
    {
        ...
        description: "Quote the advance (earned-wage loan) terms for a recipient: their LTV-bps tier, APR-bps rate (priced to reputation), max available advance against a specific payroll, and current outstanding debt.",
        inputSchema: z.object({
            chainId: ChainIdSchema,
            recipient: AddressSchema,
            payrollId: PayrollIdSchema,
        ...
        handler: async (args) => {
            const { chainId, recipient, payrollId } = args;
    ...
    },
    {
        ...
        description: "Read the yield-bearing escrow state for a payroll: available balance and accrued (not-yet-claimed) yield.",
        inputSchema: z.object({
            chainId: ChainIdSchema,
            payrollId: PayrollIdSchema,
        }),
    ...
    },
    {
        ...
        description: "For a recipient in STREAM or HYBRID cadence, returns how much has accrued but not yet been claimed for a given payroll.",
        inputSchema: z.object({
            chainId: ChainIdSchema,
            payrollId: PayrollIdSchema,
            recipient: AddressSchema,
        ...
        handler: async (args) => {
            const { chainId, payrollId, recipient } = args;
    ...
    },
    {
        ...
        description: "List the compliance hooks attached to a payroll, and preview whether a prospective payment would pass them (with a human-readable reason if it would fail).",
        inputSchema: z.object({
            chainId: ChainIdSchema,
            payrollId: PayrollIdSchema,
            recipient: AddressSchema,
        ...
        handler: async (args) => {
            const { chainId, payrollId, recipient, amount } = args;
    ...
    },
    {
        ...
        description: "Read the on-chain salary oracle (Chainlink-compatible) for a role+region — e.g. role='SWE', region='US'. Returns the index price and last-update timestamp.",
        inputSchema: z.object({
            chainId: ChainIdSchema,
            role: z.string(),
            region: z.string(),
        ...
        handler: async (args) => {
            const { chainId, role, region } = args;
    ...
    },
    {
        ...
        description: "Read ERC-20 metadata (symbol, decimals) plus balance and allowance (vs. a given spender) for a specific holder. Useful for checking MockUSDT funds before creating payrolls.",
        inputSchema: z.object({
            chainId: ChainIdSchema,
            token: AddressSchema,
        ...
        handler: async (args) => {
            const { chainId, token, holder, spender } = args;
            const pc = publicClient(chainId);
            const promises = [
            ...
            if (holder)
                promises.push(pc.readContract({ address: token, abi: ERC20_ABI, functionName: "balanceOf", args: [holder] }));
            if (holder && spender)
                promises.push(pc.readContract({ address: token, abi: ERC20_ABI, functionName: "allowance", args: [holder, spender] }));

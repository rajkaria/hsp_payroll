...
export const DEFAULT_CHAIN_ID = Number(process.env.HASHPAY_DEFAULT_CHAIN ?? "11155111");
export function allowedChainIds() {
...
export function getChainEntry(chainId) {
    const entry = CHAIN_REGISTRY[chainId];
    if (!entry)
        throw new Error(`Unknown chain id: ${chainId}`);
    if (!allowedChainIds().includes(chainId)) {
        throw new Error(`Chain ${chainId} not in HASHPAY_ALLOWED_CHAINS allowlist`);
    }
    return entry;
}
export function requireProtocol(chainId, key) {
    const { protocol, status } = getChainEntry(chainId);
    const addr = protocol[key];
    if (!addr) {
        throw new Error(`${key} not deployed on chain ${chainId} (status: ${status}). ` +
            `Run contracts/scripts/deploy-protocol.ts on this network to enable.`);
    }
    return addr;
}
export function requireCore(chainId, key) {
...
export function explorerUrl(chainId, txHash) {

...
async function ctx(chainId: number) {
  ...
  return {
    ...
    pc,
    chain: entry.chain,
    explorerTxUrl: (h: string) => explorerUrl(chainId, h),
  };
}
...
export const agentTools: Tool[] = [
  {
    ...
    description:
      "One-shot setup for an agent manager: creates a payroll in STREAM cadence (per-second accrual), funds it with the given budget, and returns the teamId. Recipients start empty — add sub-agents with hashpay_hire_agent.",
    ...
    handler: async (args) => {
      ...
      for (const log of receipt.logs) {
        try {
          ...
          if (decoded.eventName === "PayrollCreated") {
            teamId = (decoded.args as { payrollId: bigint }).payrollId;
            break;
          }
        } catch {
          /* ignore */
      ...
      if (teamId === undefined) throw new Error("Could not parse teamId from receipt");
...

  {
    ...
    description:
      "Add a sub-agent to an existing team (payroll). Sets their per-cycle rate and optionally their cadence mode (defaults to the team's default — typically STREAM so they earn per-second).",
    inputSchema: z.object({
      chainId: ChainIdSchema,
    ...
    handler: async (args) => {
      ...
      if (addRes.txHash) await c.pc.waitForTransactionReceipt({ hash: addRes.txHash });
...

  {
    ...
    description:
      "Atomic 'agent finished a task, pay them now' flow. Calls executeCycle (triggers settlement under active cadence) then attests the cycle on EAS (updates ReputationRegistry). The result includes the new reputation snapshot for the agent.",
    inputSchema: z.object({
      chainId: ChainIdSchema,
      teamId: PayrollIdSchema,
      agent: AddressSchema,
    ...
    handler: async (args) => {
      ...
      if (execRes.txHash) await c.pc.waitForTransactionReceipt({ hash: execRes.txHash });
      ...
      const token = details[1] as `0x${string}`;
      const cycleNumber = details[8] as bigint;
...

  {
    ...
    description:
      "Agent-initiated: a sub-agent borrows a just-in-time advance against its expected earnings to pay for mid-task expenses (API/compute). Must be called by the agent itself (the active signer must be the sub-agent wallet).",
    inputSchema: z.object({
      chainId: ChainIdSchema,
      teamId: PayrollIdSchema,
      amount: AmountSchema,
...

  {
    ...
    description:
      "Remove a sub-agent from a team. Requires the zero-based index of the agent in the recipients array — fetch it via hashpay_get_payroll.",
    inputSchema: z.object({
      chainId: ChainIdSchema,
      teamId: PayrollIdSchema,
    ...
    handler: async (args) => {
      const a = args as { chainId: number; teamId: bigint; agentIndex: number };
      const c = await ctx(a.chainId);
